...
for i in os.listdir("src/res/pdf"):
    ...
    if i.startswith("~$"):
        continue
    fulpath = os.path.abspath(os.path.join("src\\res\\pdf", i))
    ...
    print("SAVE\t", i)
    doc.SaveAs(os.path.abspath(os.path.join("src\\news\\newspaper", os.path.basename(fulpath).replace(".pdf", ".html"))), FileFormat=8)
    print("CLOSE\t", i)
    doc.Close()
    print("OK\t",i)
    #pool.submit(convert_pdf_2_html, os.path.abspath(os.path.join("src\\res\\pdf", i)))
print("ALL DONE")
word.Quit()
print("EXITING")