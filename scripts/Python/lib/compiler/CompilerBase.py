...
class CompilerPool:
    ...
    def warp_func(self, context, func, *args, **kwargs):
        ...
        scount = f"{self.finished}/{self.total}"
        scount = stringlib.special_text(scount, "34m")
        sname = stringlib.ljust(context["name"], 10)
        sname = stringlib.special_text(sname, "33m")
        ...
        if error is None:
            self.print(scount, sname, stime, spath,sep="\t")
        else:
            self.print(scount, sname, stringlib.special_text("ERROR", "31m"), spath, "\n", error, sep="\t")
