 ...
 * Original file: /npm/pinyin-pro@3.27.0/dist/index.js
 *
...
!(function (n, i) {
	...
	Object.keys(c).forEach((n) => {
		const i = c[n];
		for (let g of i) j.set(g, n);
	});
	const d = ['zh', 'ch', 'sh', 'z', 'c', 's', 'b', 'p', 'm', 'f', 'd', 't', 'n', 'l', 'g', 'k', 'h', 'j', 'q', 'x', 'r', 'y', 'w', ''],
		e = ['j', 'q', 'x'],
		r = ['uān', 'uán', 'uǎn', 'uàn', 'uan', 'uē', 'ué', 'uě', 'uè', 'ue', 'ūn', 'ún', 'ǔn', 'ùn', 'un', 'ū', 'ú', 'ǔ', 'ù', 'u'],
		b = {
			uān: 'üān',
			uán: 'üán',
			uǎn: 'üǎn',
		...
		x = ['ia', 'ian', 'iang', 'iao', 'ie', 'iu', 'iong', 'ua', 'uai', 'uan', 'uang', 'ue', 'ui', 'uo', 'üan', 'üe', 'van', 've'],
		m = {
			一: 'yì',
			二: 'èr',
			三: 'sān',
	...
	const w = (function () {
		...
		f = Object.keys(w).map((n) => ({ zh: n, pinyin: w[n], probability: 1e-12, length: z(n), priority: s, dict: Symbol('rule') })),
		q = { 不: { bú: [4] }, 一: { yí: [4], yì: [1, 2, 3] } },
		a = { 不: ['的', '而', '之', '后', '也', '还', '地'], 一: ['的', '而', '之', '后', '也', '还', '是'] },
		k = Object.keys(q);
	function v(n, i, g) {
		return (
			(function (n, i) {
				if ('々' === n) return i && j.get(i) ? j.get(i).split(' ')[0] : 'tóng';
			})(n, i) ||
			(function (n, i) {
				if (!('了' !== n || (i && j.get(i)))) return 'liǎo';
			})(n, i) ||
			(function (n, i, g) {
				if (-1 === k.indexOf(n)) return X(n);
				if (i === g && i && X(i) !== i) return nn(X(n));
				if (g && !a[n].includes(g)) {
					const i = X(g);
					if (i !== g) {
						const g = sn(i),
	...
	const Z = {
		...
		P = Object.keys(Z).map((n) => ({ zh: n, pinyin: Z[n], probability: 1 + z(n), length: z(n), priority: o, dict: Symbol('surname') })),
		O = {
			这个: 'zhè ge',
			成为: 'chéng wéi',
			认为: 'rèn wéi',
		...
		S = Object.keys(O).map((n) => ({ zh: n, pinyin: O[n], probability: 2e-8, length: 2, priority: s, dict: Symbol('dict2') })),
		C = {
			为什么: 'wèi shén me',
			实际上: 'shí jì shang',
			检察长: 'jiǎn chá zhǎng',
		...
		T = Object.keys(C).map((n) => ({ zh: n, pinyin: C[n], probability: 2e-8, length: 3, priority: s, dict: Symbol('dict3') })),
		A = {
		...
		F = Object.keys(A).map((n) => ({ zh: n, pinyin: A[n], probability: 2e-8, length: 4, priority: s, dict: Symbol('dict4') })),
		E = {
		...
		$ = Object.keys(E).map((n) => ({ zh: n, pinyin: E[n], probability: 2e-8, length: 5, priority: s, dict: Symbol('dict5') }));
	function D(n, i) {
	...
	function N(n) {
		return n.priority === t ? -n.length * n.length * 100 : n.priority === o ? -n.length * n.length * 10 : 0;
	}
	function I(n, i) {
		return n && n.count <= i.count ? n : i;
	}
	function R(n) {
		return n.priority === t ? -n.length * n.length * 1e5 : n.priority === o ? -n.length * n.length * 100 : 1;
	}
	function B(n, i) {
	...
	const L = [...$, ...F, ...T, ...S, ...f, ...P],
		Q = new (class {
			constructor() {
				((this.dictMap = new Map()), (this.queues = []), (this.root = new H(null)));
			}
			build(n) {
			...
			buildTrie(n) {
				for (let i of n) {
					...
					for (let i = 0; i < n.length; i++) {
						let h = n[i];
						if (!g.children.has(h)) {
							const u = new H(g, n.slice(0, i).join(''), h);
			...
			addNodeToQueues(n) {
				(this.queues[z(n.prefix)] || (this.queues[z(n.prefix)] = []), this.queues[z(n.prefix)].push(n));
			}
			insertPattern(n, i) {
			...
			match(n, i) {
				...
				for (let n = 0; n < u.length; n++) {
					let s = u[n];
					for (; null !== g && !g.children.has(s); ) g = g.fail;
					if (null === g) g = this.root;
					else {
						g = g.children.get(s);
						const u = g.patterns.find((g) => ('off' === i ? g.priority !== o : 'head' !== i || g.length - 1 - n == 0));
						u && h.push(Object.assign(Object.assign({}, u), { index: n - u.length + 1 }));
						let t = g.fail;
						for (; null !== t; ) {
							const g = t.patterns.find((g) => ('off' === i ? g.priority !== o : 'head' !== i || g.length - 1 - n == 0));
							(g && h.push(Object.assign(Object.assign({}, g), { index: n - g.length + 1 })), (t = t.fail));
						}
					}
				}
				return h;
					...
					: 3 === g
						? (function (n, i) {
								...
								for (let s = i - 1; s >= 0; s--) {
									...
									for (; u && u.index + u.length - 1 === s; ) {
										const i = u.index,
											s = { count: R(u) + o.count, patterns: o.patterns, concatPattern: u };
										((g[i] = I(g[i], s)), (u = n[--h]));
									}
									const t = { count: 1 + o.count, patterns: o.patterns };
									((g[s] = I(g[s], t)),
										g[s].concatPattern &&
						...
						: (function (n, i) {
								...
								let h = n.length - 1,
									u = n[h];
								for (let s = i - 1; s >= 0; s--) {
									const o = s + 1 >= i ? { probability: 1, decimal: 0, patterns: [] } : g[s + 1];
									for (; u && u.index + u.length - 1 === s; ) {
										const i = u.index,
											s = {
												probability: u.probability * o.probability,
												decimal: o.decimal + N(u),
												patterns: o.patterns,
												concatPattern: u,
											};
										(M(s), (g[i] = D(g[i], s)), (u = n[--h]));
									}
									const t = { probability: 1e-13 * o.probability, decimal: 0, patterns: o.patterns };
									(M(t),
										(g[s] = D(g[s], t)),
										g[s].concatPattern &&
											((g[s].patterns = g[s].patterns.concat(g[s].concatPattern)),
	...
	function K(n, i, g) {
		for (let h in n) {
			...
			y(h).forEach((n, h) => {
				const s = u.split(' ')[h] || '';
				'replace' === g || ('add' === g && !i.get(n) && !j.get(n))
					? i.set(n, s)
					: (i.set(n, i.get(n) || j.get(n)), i.get(n).split(' ').includes(s) || i.set(n, `${i.get(n)} ${s}`.trim()));
			});
		}
	}
	const V = () => G;
	const X = (n) => {
		...
		Y = (n, i, g, h) => {
			const u = Q.search(n, g, h);
			let o = 0;
			const t = y(n);
			for (let n = 0; n < t.length; ) {
				const g = u[o];
				if (g && n === g.index) {
					if (1 === g.length && g.priority <= s) {
						const g = t[n];
						let h = '';
						((h = v(g, t[n - 1], t[n + 1])), (i[n] = { origin: g, result: h, isZh: h !== g, originPinyin: h }), n++, o++);
						continue;
					}
					...
					for (let s = 0; s < g.length; s++) {
						const o = y(g.zh);
						((i[n + s] = { origin: o[s], result: h[u] || '', isZh: !0, originPinyin: h[u] || '' }), u++);
					}
					((n += g.length), o++);
				} else {
					const g = t[n];
					let h = '';
					((h = v(g, t[n - 1], t[n + 1])), (i[n] = { origin: g, result: h, isZh: h !== g, originPinyin: h }), n++);
				}
			}
		...
		hn = (n, i) => {
			...
			for (let n of g)
				for (let i of d)
					if (n.startsWith(i)) {
						let g = n.slice(i.length);
						(-1 !== e.indexOf(i) && -1 !== r.indexOf(g) && (g = b[g]), h.push(i), u.push(g));
						break;
					}
			return (
				'standard' === i &&
		...
		un = (n) => {
			...
			let g = '',
				...
				u = '';
			return (
				-1 !== x.indexOf(nn(i)) ? ((g = i[0]), (h = i[1]), (u = i.slice(2))) : ((h = i[0] || ''), (u = i.slice(1) || '')),
		...
		tn = (n) => 'string' == typeof n || (console.error('The first param of pinyin is error: ' + n + ' is not assignable to type "string".'), !1);
	function zn(n, i) {
		return !(i instanceof RegExp) || i.test(n);
	}
	const yn = (n, i) => {
			let g = i.nonZh;
			if ('removed' === g) return n.filter((n) => n.isZh || !zn(n.origin, i.nonZhScope));
			if ('consecutive' === g) {
				for (let g = n.length - 2; g >= 0; g--) {
					...
					!h.isZh &&
						!u.isZh &&
						zn(h.origin, i.nonZhScope) &&
						zn(u.origin, i.nonZhScope) &&
		...
		cn = (n, i) => {
			switch (i.pattern) {
				...
				case 'num':
					n.forEach((n) => {
						n.result = n.isZh ? sn(n.result) : '';
					});
					break;
				case 'initial':
					n.forEach((n) => {
						n.result = n.isZh ? hn(n.result, i.initialPattern).initial : '';
					});
					break;
				case 'final':
					n.forEach((n) => {
						n.result = n.isZh ? hn(n.result, i.initialPattern).final : '';
					});
					break;
				case 'first':
					n.forEach((n) => {
						n.result = on(n.result, n.isZh);
					});
					break;
				case 'finalHead':
					n.forEach((n) => {
						n.result = n.isZh ? un(n.result).head : '';
					});
					break;
				case 'finalBody':
					n.forEach((n) => {
						n.result = n.isZh ? un(n.result).body : '';
					});
					break;
				case 'finalTail':
					n.forEach((n) => {
						n.result = n.isZh ? un(n.result).tail : '';
					});
			}
		},
		jn = (n, i) => {
		...
		dn = (n, i) => {
			i.v &&
				n.forEach((n) => {
					n.isZh && (n.result = n.result.replace(/ü/g, 'string' == typeof i.v ? i.v : 'v'));
				});
		},
		en = (n, i) => (
			!1 === i &&
	...
	function bn(n, i) {
		i = Object.assign(Object.assign({}, rn), i || {});
		if (!tn(n)) return n;
		...
		let g = Array(z(n)),
			{ list: h } = Y(n, g, i.surname, i.segmentit);
		return (
			(h = en(h, i.toneSandhi)),
			(h = yn(h, i)),
			ln(n, i) && (h = ln(n, i)),
					...
					: 'all' === i.type
						? n.map((n) => {
								const g = n.isZh ? n.result : '',
									{ initial: h, final: u } = hn(g, i.initialPattern),
									{ head: s, body: o, tail: t } = un(g);
								let z = [];
								return (
									'' !== g && (z = [g].concat(gn(n.origin, i.surname).filter((n) => n !== g))),
									{
										origin: n.origin,
										pinyin: g,
										initial: h,
										final: u,
										first: on(n.result, n.isZh),
										finalHead: s,
										finalBody: o,
										finalTail: t,
										num: Number(sn(n.originPinyin)),
										isZh: n.isZh,
										polyphonic: z,
										inZhRange: !!j.get(n.origin),
										result: n.result,
									}
								);
							})
	...
	const xn = Symbol('default'),
		mn = new Map();
	function pn(n, i, g, h = 'add') {
		mn.get(n) || mn.set(n, {});
		const u = mn.get(n);
		if ((u[i] || (u[i] = j.get(i)), 'add' === h)) {
			const n = j.get(i);
			n && !n.split(' ').includes(g) ? j.set(i, `${n} ${g}`) : j.get(i) || j.set(i, g);
		} else 'replace' === h && j.set(i, g);
	}
	const wn = { precision: 'first', continuous: !1, space: 'ignore', lastPrecision: 'start', insensitive: !0, v: !1 },
		fn = (n, i) => {
			let g = 0;
		...
		qn = (n, i, g) => {
			...
			for (let n = 0; n < u.length; n++) {
				...
				const o = bn(u[n], { toneType: 'none', multiple: !0, type: 'array', v: g.v });
				let t = 0;
				if (
					(o.forEach((n) => {
						const g = fn(n, i);
		...
		an = (n, i, g) => {
			...
			for (let n = 1; n < u.length; n++) {
				if (!g.continuous || ('ignore' == g.space && ' ' === h[n - 1])) for (let g = 1; g <= i.length; g++) u[n][g - 1] = u[n - 1][g - 1];
				for (let s = 1; s <= i.length; s++)
					if (u[n - 1][s - 1] && (1 === s || u[n - 1][s - 1].length)) {
						const o = bn(h[n - 1], { type: 'array', toneType: 'none', multiple: !0, v: g.v });
						if (h[n - 1] === i[s - 1]) {
						...
						if (
							...
							'first' === t && o.some((n) => n[0] === i[s - 1]))
						) {
							const i = [...u[n - 1][s - 1], n - 1];
							(!u[n][s] || i.length > u[n][s].length) && (u[n][s] = i);
						}
						const z = o.find((n) => n === i.slice(s - 1, s - 1 + n.length));
						if (z) {
							const i = [...u[n - 1][s - 1], n - 1],
								g = s - 1 + z.length;
	...
	const kn = {
		...
		vn = { pattern: 'pinyin', toneType: 'symbol', type: 'string', v: !1, nonZh: 'spaced' };
	const Zn = (n) =>
			y(n).map((n) => {
				const i = V().get(n) || j.get(n) || n;
				return { origin: n, result: i, isZh: i !== n, originPinyin: i };
			}),
		Pn = (n) => n.map((n) => (n.isZh ? n.result.split(' ').map((i) => ({ origin: n.origin, result: i, isZh: !0, originPinyin: i })) : [n])),
		On = (n, i) =>
			'array' === i.type
	...
	((n.OutputFormat = void 0),
		((Tn = n.OutputFormat || (n.OutputFormat = {}))[(Tn.AllSegment = 1)] = 'AllSegment'),
		(Tn[(Tn.AllArray = 2)] = 'AllArray'),
		(Tn[(Tn.AllString = 3)] = 'AllString'),
		(Tn[(Tn.PinyinSegment = 4)] = 'PinyinSegment'),
		(Tn[(Tn.PinyinArray = 5)] = 'PinyinArray'),
		(Tn[(Tn.PinyinString = 6)] = 'PinyinString'),
		(Tn[(Tn.ZhSegment = 7)] = 'ZhSegment'),
		(Tn[(Tn.ZhArray = 8)] = 'ZhArray'),
		(Tn[(Tn.ZhString = 9)] = 'ZhString'));
	const An = {
		toneType: 'symbol',
		mode: 'normal',
		nonZh: 'spaced',
	...
	((n.addDict = function (n, i) {
		const g = [],
			h = ('object' == typeof i ? i.name : i) || xn,
			u = (null == i ? void 0 : i.dict1) || 'add';
		for (let i in n) {
			const o = n[i],
				t = Array.isArray(o) ? o[0] : o,
				y = z(i);
			(1 === y && pn(h, i, t, u),
				Array.isArray(o)
					? g.push({
							zh: i,
							pinyin: t,
							probability: 'number' == typeof o[1] ? o[1] : 2e-8 * y * y,
							length: y,
							priority: s,
							dict: h,
							pos: 'string' == typeof o[2] ? o[2] : '',
						})
					: g.push({ zh: i, pinyin: t, probability: 2e-8 * y * y, length: y, priority: s, dict: h }));
		}
		Q.build(g);
	}),
		(n.clearCustomDict = function (n) {
			(('pinyin' !== n && -1 === n.indexOf('pinyin')) ||
				...
				Q.removeDict(J)),
				('multiple' !== n && -1 === n.indexOf('multiple')) || _.clear(),
				('polyphonic' !== n && -1 === n.indexOf('polyphonic')) || G.clear());
		}),
				...
				(n = n.map((n) => {
					...
					return 'numToSymbol' === g
						? (function (n) {
								...
								if (i >= 0 && i <= 4) {
									for (let g in Cn) if (n.includes(g)) return n.slice(0, n.length - 1).replace(g, Cn[g][i]);
									return n;
								}
								return n;
							})(n)
		...
		(n.customPinyin = function (n = {}, i) {
			...
			const g = Object.keys(U).map((n) => ({ zh: n, pinyin: U[n], probability: 1 + z(n), length: z(n), priority: t, dict: J }));
			(Q.build(g), (null == i ? void 0 : i.multiple) && K(n, _, i.multiple), (null == i ? void 0 : i.polyphonic) && K(n, G, i.polyphonic));
		}),
		(n.getFinalParts = un),
		(n.getInitialAndFinal = hn),
		(n.getNumOfTone = sn),
		(n.html = (n, i) => {
			const g = Object.assign(Object.assign({}, kn), i || {});
			return bn(n, { type: 'all', toneType: g.toneType, toneSandhi: null == i ? void 0 : i.toneSandhi, v: g.v })
				.map((n) => {
					let i = '';
					...
					if (n.isZh) {
						const h = g.resultClass || kn.resultClass,
							u = g.chineseClass || kn.chineseClass,
							s = g.pinyinClass || kn.pinyinClass;
						return `<span class="${h}${i}"><ruby><span class="${u}">${n.origin}</span>${g.rp ? '<rp>(</rp>' : ''}<rt class="${s}">${n.pinyin}</rt>${g.rp ? '<rp>)</rp>' : ''}</ruby></span>`;
					}
					if (g.wrapNonChinese) {
						return `<span class="${g.nonChineseClass || kn.nonChineseClass}${i}">${n.origin}</span>`;
					}
					return n.origin;
				})
				.join('');
		...
		(n.match = (n, i, g) => {
			...
			const h = Object.assign(Object.assign({}, wn), g || {});
			...
			return (function (n, i) {
				...
				for (let n = 0; n < i.length; n++) {
					...
					const t = o + h;
					2 === u[o].length ? g.push(t - 1, t) : g.push(t);
				}
				return g;
			})(n, 'any' === (null == g ? void 0 : g.precision) ? qn(n, i, h) : an(n, i, h));
		}),
		(n.pinyin = bn),
		(n.polyphonic = function (n, i = vn) {
			if (!tn(n)) return [];
			if ('' === n) return [];
			('all' === i.type && (i.pattern = 'pinyin'), 'num' === i.pattern && (i.toneType = 'none'), i.removeNonZh && (i.nonZh = 'removed'));
			let g = Zn(n);
			g = yn(g, i);
			let h = Pn(g);
			(h.forEach((n) => {
			...
			return h.map((n) => On(n, i));
		}),
		(n.removeDict = function (n) {
			(Q.removeDict(n || xn),
				(function (n) {
					const i = mn.get(n) || {};
					for (let n in i) (j.set(n, i[n]), delete i[n]);
				})(n || xn));
		}),
		(n.segment = function (i, g) {
			if (((g = Object.assign(Object.assign({}, An), g || {})), !tn(i))) return i;
			void 0 === g.surname && ('surname' === g.mode ? (g.surname = 'all') : (g.surname = 'off'));
			let h = Array(z(i)),
				{ list: u, matches: s } = Y(i, h, g.surname, g.segmentit);
			((u = en(u, g.toneSandhi)), (u = yn(u, g)), jn(u, g), dn(u, g));
			const o = (function (n, i) {
				...
				for (; h < n.length && u < i.length; ) {
					...
					if (s.zh.startsWith(o.origin)) {
						...
						const z = n.slice(i, t);
						(g.push({ segment: z.map((n) => ({ origin: n.origin, result: n.result })), isZh: !0 }), (h += t - i), u++);
					} else (g.push({ segment: [{ origin: o.origin, result: o.result }], isZh: !1 }), h++);
				}
				for (; h < n.length; ) {
					const i = n[h];
					(g.push({ segment: [{ origin: i.origin, result: i.result }], isZh: !1 }), h++);
				}
				return g;
			})(u, s);
			return (function (i, g) {
				...
				if (h === n.OutputFormat.AllSegment)
					return i.map((n) => ({ origin: n.segment.map((n) => n.origin).join(''), result: n.segment.map((n) => n.result).join('') }));
				if (h === n.OutputFormat.AllArray) return i.map((n) => n.segment);
				if (h === n.OutputFormat.AllString) {
					const n = i.map((n) => ({ origin: n.segment.map((n) => n.origin).join(''), result: n.segment.map((n) => n.result).join('') }));
					return { origin: n.map((n) => n.origin).join(u), result: n.map((n) => n.result).join(u) };
				}
							...
							: h === n.OutputFormat.ZhSegment
								...
								: h === n.OutputFormat.ZhArray
									...
									: h === n.OutputFormat.ZhString
										...
										: void 0;
			})(o, { format: g.format, separator: g.separator });
		}),
